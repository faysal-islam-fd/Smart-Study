...
router.post('/mindmap/:documentId', async (req, res) => {
  try {
    const document = await Document.findById(req.params.documentId);
    if (!document) {
      return res.status(404).json({ error: 'Document not found' });
    ...
    if (document.mindMap) {
      return res.json({ mindMap: document.mindMap });
    }
...
router.post('/all/:documentId', async (req, res) => {
  try {
    ...
    const [summary, flashcards, quiz, mindMap] = await Promise.all([
      ...
      document.mindMap || generateMindMap(document.extractedText)
    ]);
    ...
    res.json({ summary, flashcards, quiz, mindMap });
  } catch (error) {
