...
Now, analyze the content above and create a well-formatted, comprehensive summary following these guidelines.`;
  ...
  // Free tier: gemini-2.5-flash (250 req/day), gemini-2.5-pro (100 req/day)
...
async function generateMindMap(text, fileBuffer = null, fileType = null, fileName = null, useFileUpload = false) {
  const genAIInstance = getGenAI();
  ...
  // If file upload is required (scanned PDF), use it
  if (useFileUpload && fileBuffer && fileType) {
    console.log('📤 Preparing file for mind map generation (scanned PDF)...');
    filePart = prepareFileDataForGemini(fileBuffer, fileType);
    
    prompt = `You are an expert educational assistant specializing in creating hierarchical mind maps. Analyze this document and create a well-organized mind map structure that visually represents the relationships between concepts.

CRITICAL INSTRUCTIONS:
1. CONTENT ANALYSIS: Carefully read and analyze ALL text, images, diagrams, and visual content in the document
2. IGNORE ARTIFACTS: Completely ignore watermarks, app names (like "CamScanner"), filenames, page numbers, or any metadata - focus ONLY on educational content
3. MIND MAP STRUCTURE:
   - Identify the main central topic or theme
   - Break down into major subtopics and sections
   - Further decompose into key concepts, facts, and details
   - Show clear hierarchical relationships (parent-child connections)
   - Organize logically to show how concepts relate to each other

4. CONTENT QUALITY:
   - Extract the most important and relevant information
   - Use clear, concise labels (2-5 words per node)
   - Ensure all information is accurate and based on the document
   - Create a balanced structure (not too deep, not too shallow - typically 2-4 levels)

5. FORMAT: Return ONLY a valid JSON object with this exact structure:
   {
     "id": "root",
     ...
     "children": [
       {
         "id": "topic1",
         "label": "Major Subtopic 1",
         "children": [
           {
             "id": "concept1",
             "label": "Key Concept or Detail",
             "children": []
           }
         ]
       },
...
6. REQUIREMENTS:
   - Root node must have "id": "root" and a descriptive "label"
   - Each child must have a unique "id" (string), a "label" (string), and a "children" array
   - Children arrays can be empty for leaf nodes
   - Use meaningful, descriptive labels
   - Create 3-6 major subtopics with 2-5 sub-concepts each

7. ACCURACY: Ensure all information is accurate and directly based on the document content

Now, analyze the document and create a well-structured mind map. Return ONLY the JSON object, no additional text, explanations, or markdown formatting.`;
    
    console.log('✅ Using Gemini multimodal API for mind map');
  } else {
    // Use text extraction (original method)
    if (!text || text.trim().length === 0) {
      throw new Error('No text content provided to generate mind map');
    }
    ...
    prompt = `You are an expert educational assistant specializing in creating hierarchical mind maps. Analyze the following academic content and create a well-organized mind map structure that visually represents the relationships between concepts.
...
3. CONTENT QUALITY:
   - Extract the most important and relevant information
   - Use clear, concise labels (2-5 words per node)
   - Ensure all information is accurate and based on the content
   - Create a balanced structure (not too deep, not too shallow - typically 2-4 levels)

4. FORMAT: Return ONLY a valid JSON object with this exact structure:
   {
     "id": "root",
     "label": "Main Topic/Title",
     "children": [
...
6. ACCURACY: Ensure all information is accurate and directly based on the content provided

Content:
${textToProcess.substring(0, 15000)}

Now, analyze the content above and create a well-structured mind map. Return ONLY the JSON object, no additional text, explanations, or markdown formatting.`;
  }
  ...
  for (let i = 0; i < models.length; i++) {
    const modelName = models[i];
    try {
      console.log(`📝 Trying model ${modelName} for mind map...`);
      const model = genAIInstance.getGenerativeModel({ model: modelName });
      ...
      console.error(`Error generating mind map with ${modelName}:`, error);
      if (errorMsg.includes('403') || error.code === 403) {
        throw new Error('API key is invalid or restricted. Please check your Gemini API key and permissions.');
      }
      throw error;
  ...
  throw new Error('Failed to generate mind map. No available models found.');
}
